...
%               this argument should be set to TI2-TI1 in QUIPSS. In Siemens PICORE sequence, TI1/TI2 can be found by the "inversion time 1" and "inversion time 2"
...
% close all;
spmver = spm('ver',[],1);
modernSPM = any(strcmp(spmver,{'SPM5','SPM8','SPM12'}));
...
if CBFFlag==1,
   ...
   if AslType==0
      useM0=1;                         % PASLUSEM0
   elseif AslType==1 
      if QuantFlag==1, useM0=1;      % CASLUSEM0VAL
      else  % mapwise M0 or M0ctr calibration
      	if exist('M0img','var')==0   % M0 image has not been provided 
      		pos=pos+1;
      		useM0= spm_input('Use an additionally acquired M0 rather than M0ctr for CASL?', pos, 'm', 'yes|*no', [1 0], 0);     % CASLUSEM0
    ...
    if QuantFlag ==1  % useM0 has to be 1 (see lines 367 PASLUSEM0 and 376 CASLUSEM0VAL). need explicitly input unique M0 value or implicitly defined M0 value using M0image and M0roi
        ...
        else  % if M0img exists but M0 value has been provided, M0img will be set to [], since M0value has higher priority
...
for p=1:perfno
   ...
   MB=2;     % change this according to your protocol
   NB=ceil(Vall(1).dim(3)/MB);    % the number of slices is often an even number in MB imaging
   sldelay=(0:NB-1) * Slicetime;
   sldelay=repmat(sldelay, 1, MB);   % copy for MB timesslicetimearray = repmat(sldelay, Vall(1).dim(1)*Vall(1).dim(2), 1);
...
%    BOLDimg=Vconimg;
   ...
   if CBFFlag==1
      if AslType==0  % PASL
             ...
             tmpsig=meanbold(brain_ind);
             tcbf=zeros(size(brain_ind));
